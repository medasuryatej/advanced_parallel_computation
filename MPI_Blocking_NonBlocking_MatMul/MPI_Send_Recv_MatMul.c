    ...
    if (proc_rank == MAIN_PROC) {
        ...
        */
        ...
        for (int proc_num=1; proc_num <= parallel_procs; proc_num++) {
            ...
            MPI_Send(&starting_offset, 1, MPI_INT, proc_num, TAG_FROM_MAIN, MPI_COMM_WORLD);
            MPI_Send(&row_transfer, 1, MPI_INT, proc_num, TAG_FROM_MAIN, MPI_COMM_WORLD);
            // send the address of the buffer
            // MPI_Send(&matA[starting_offset][0], row_transfer*DIM, MPI_DOUBLE, proc_num, tag, MPI_COMM_WORLD);
            MPI_Send(&m1[starting_offset][0], row_transfer*DIM, ComplexNumberType, proc_num, TAG_FROM_MAIN, MPI_COMM_WORLD);
        ...
        for (int proc_num=1; proc_num <= parallel_procs; proc_num++) {
            ...
            MPI_Recv(&starting_offset, 1, MPI_INT, sender_proc, TAG_FROM_OTHER, MPI_COMM_WORLD, &status);
            MPI_Recv(&row_transfer, 1, MPI_INT, sender_proc, TAG_FROM_OTHER, MPI_COMM_WORLD, &status);
            ...
            MPI_Recv(&m3[starting_offset][0], row_transfer*DIM, ComplexNumberType, sender_proc, TAG_FROM_OTHER, MPI_COMM_WORLD, &status);
        }
       ...
       printf("Time taken for MatMul with Dimensions: %d is %f\n", DIM, (end-start));
    }
    else {
        // parallel procs
        ...
        tag = 100;
        // receive offsets
        MPI_Recv(&starting_offset, 1, MPI_INT, sender_proc, TAG_FROM_MAIN, MPI_COMM_WORLD, &status);
        MPI_Recv(&row_transfer, 1, MPI_INT, sender_proc, TAG_FROM_MAIN, MPI_COMM_WORLD, &status);
        ...
        MPI_Recv(&m1, row_transfer*DIM, ComplexNumberType, sender_proc, TAG_FROM_MAIN, MPI_COMM_WORLD, &status);
        ...
        */
        ...
        MPI_Send(&starting_offset, 1, MPI_INT, MAIN_PROC, TAG_FROM_OTHER, MPI_COMM_WORLD);
        MPI_Send(&row_transfer, 1, MPI_INT, MAIN_PROC, TAG_FROM_OTHER, MPI_COMM_WORLD);
        ...
        MPI_Send(&m3, row_transfer*DIM, ComplexNumberType, MAIN_PROC, TAG_FROM_OTHER, MPI_COMM_WORLD);
    }
